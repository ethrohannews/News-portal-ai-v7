        ...
        <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="space-y-4 md:space-y-8">
          ...
          <TabsContent value={selectedCategory} className="space-y-4 md:space-y-6">
            ...
            {featuredNews.length > 0 && selectedCategory === 'সব' && (
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  <Star className={`w-5 h-5 md:w-6 md:h-6 ${isDarkMode ? 'text-yellow-400' : 'text-yellow-500'}`} />
                  <h2 className={`text-xl md:text-2xl font-bold ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                    ফিচার সংবাদ
                  </h2>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                  {featuredNews.map((article) => (
                    <Card 
                      key={article.id} 
                      className={`group hover:shadow-xl transition-all duration-300 cursor-pointer shadow-lg ${
                        isDarkMode 
                          ? 'bg-gradient-to-br from-slate-800 to-slate-700 border-yellow-500/30' 
                ...
                {news.map((article) => (
                  ...
                  >
                    <div className="p-4 md:p-6">
                      ...
                      <h3 className={`text-base md:text-lg font-bold mb-3 group-hover:text-orange-600 transition-colors line-clamp-2 ${
                        isDarkMode ? 'text-slate-200' : 'text-slate-800'
                      }`}>
                        {article.title}
                      </h3>
